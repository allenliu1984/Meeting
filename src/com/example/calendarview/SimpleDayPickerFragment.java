...
/**
 * <p>
 * This displays a titled list of weeks with selectable days. It can be configured to display the week number,
 * start the week on a given day, show a reduced number of days, or display an arbitrary number of weeks at a
 * time. By overriding methods and changing variables this fragment can be customized to easily display a
 * month selection component in a given style.
 * </p>
 */
public class SimpleDayPickerFragment extends ListFragment implements OnScrollListener {
	...
	/**
	 * Creates a new adapter if necessary and sets up its parameters. Override this method to provide a custom
	 * adapter.
	 */
	...
	/**
	 * Sets up the strings to be used by the header. Override this method to use different strings or modify
	 * the view params.
	 */
	...
	/**
	 * Sets all the required fields for the list view. Override this method to set a different list view
	 * behavior.
	 */
	...
	/**
	 * Fixes the day names header to provide correct spacing and updates the label text. Override this to set
	 * up a custom header.
	 */
...
//    }
	...
	/**
	 * This moves to the specified time in the view. If the time is not already in range it will move the list
	 * so that the first of the month containing the time is at the top of the view. If the new time is
	 * already in view the list will not be scrolled unless forceScroll is true. This time may optionally be
	...
	public boolean goTo(long time, boolean animate, boolean setSelected, boolean forceScroll) {
		...
		int position = getWeeksSinceEpochFromJulianDay(Time.getJulianDay(millis, mTempTime.gmtoff),
				mFirstDayOfWeek);
		...
		if (position < firstPosition || position > lastPosition || forceScroll) {
			...
			position = getWeeksSinceEpochFromJulianDay(Time.getJulianDay(millis, mFirstDayOfMonth.gmtoff),
					mFirstDayOfWeek);
	...
	/**
	 * Sets the month displayed at the top of this view based on time. Override to add custom events when the
	 * title is changed.
	 *
	 * @param time
