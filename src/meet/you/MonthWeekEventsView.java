...
public class MonthWeekEventsView extends SimpleWeekView {
	...
	/*
	 * NOTE: these are not constants, and may be multiplied by a scale
	 * factor
	 */
	...
	// This is for drawing the outlines around event chips and supports
	// up to 10
	// events being drawn on each day. The code will expand this if
	// necessary.
	protected FloatRef mEventOutlines = new FloatRef(10 * 4 * 4 * 7);

	protected static StringBuilder mStringBuilder = new StringBuilder(
			50);
			...
			synchronized (this) {
				...
				if (mFadingIn) {
					...
					mTodayAnimator = ObjectAnimator
							.ofInt(MonthWeekEventsView.this,
									"animateTodayAlpha",
									255,
									0);
	...
	/**
	 * This provides a reference to a float array which allows for easy
	 * size checking and reallocation. Used for drawing lines.
	 */
	...
	// Sets the list of events for this week. Takes a sorted list of
	// arrays
	// divided up by day for generating the large month version and the
	// full
	...
	public void setEvents(List<ArrayList<Event>> sortedEvents,
		...
		// The MIN_WEEK_WIDTH is a hack to prevent the view from
		// trying to
		// generate dna bits before its width has been fixed.
	}

	/**
	 * Sets up the dna bits for the view. This will return early if the
	 * view isn't in a state that will create a valid set of dna yet
	 * (such as the views width not being set correctly yet).
	 */
	...
	protected void loadColors(Context context) {
		...
		mMonthNumOtherColor = res
				.getColor(R.color.month_day_number_other);
		mMonthNumTodayColor = res
				.getColor(R.color.month_today_number);
		...
		mMonthEventExtraColor = res
				.getColor(R.color.month_event_extra_color);
		mMonthEventOtherColor = res
				.getColor(R.color.month_event_other_color);
		...
		mDaySeparatorInnerColor = res
				.getColor(R.color.month_grid_lines);
	...
	/**
	 * Sets up the text and style properties for painting. Override this
	 * if you want to use a different paint.
	 */
	@Override
	protected void initView() {
		...
		if (!mInitialized) {
			Resources resources = getContext()
					.getResources();
			mShowDetailsInMonth = Utils
					.getConfigBool(getContext(),
			...
			if (!mShowDetailsInMonth) {
				TOP_PADDING_MONTH_NUMBER += DNA_ALL_DAY_HEIGHT
						+ DNA_MARGIN;
			}
			mInitialized = true;
		}
		...
		mFramedEventPaint = new TextPaint(
		...
		mEventHeight = (int) (mEventPaint.descent()
				- mEventPaint.ascent() + 0.5f);
		...
		mExtrasAscentHeight = (int) (-mEventExtrasPaint
				.ascent() + 0.5f);
		...
		mEventDeclinedExtrasPaint
				.setStrokeWidth(EVENT_SQUARE_BORDER);
		mEventDeclinedExtrasPaint.setTextSize(TEXT_SIZE_EVENT);
		mEventDeclinedExtrasPaint
				.setColor(mMonthDeclinedExtrasColor);
		...
		if (DEBUG_LAYOUT) {
			...
			Log.d("EXTRA",
					"mEventExtrasPaint ascent="
							...
							+ mExtrasHeight);
			Log.d("EXTRA",
					"mWeekNumPaint ascent="
							+ mWeekNumPaint.ascent()
							+ " descent="
							+ mWeekNumPaint.descent());
		}
	}
	...
	public void setWeekParams(HashMap<String, Integer> params, String tz) {
		...
		if (params.containsKey(VIEW_PARAMS_ANIMATE_TODAY)
				&& mHasToday) {
	...
	public boolean updateToday(String tz) {
		...
		int julianToday = Time.getJulianDay(
				mToday.toMillis(false),
				mToday.gmtoff);
		if (julianToday >= mFirstJulianDay
				&& julianToday < mFirstJulianDay
						+ mNumDays) {
			mHasToday = true;
...
//		drawPoint(canvas);
	...
	//if there are some meetings on this day ,draw a point on it.
	...
	protected void drawToday(Canvas canvas) {
		...
		r.right = computeDayLeftPosition(mTodayIndex + 1)
				- (int) Math.ceil(TODAY_HIGHLIGHT_WIDTH / 2.0f);
		p.setColor(mTodayAnimateColor
				| (mAnimateTodayAlpha << 24));
		canvas.drawRect(r, p);
	...
	protected void drawDaySeparators(Canvas canvas) {
		...
		int i = 0;
		if (mShowWeekNum) {
			// This adds the first line separating the
			// week number
			int xOffset = SPACING_WEEK_NUMBER
					+ mPadding;
			count += 4;
			lines[i++] = xOffset;
			lines[i++] = 0;
	...
	protected void drawBackground(Canvas canvas) {
		...
		int colorCount=1;
		for (int j = 0; j < mBusyDay.length; j++) {
			colorCount++;
			int pos=mBusyDay[j]-mFirstJulianDay;
			Paint paint=new Paint();
			if (j>0&&mBusyDay[j]==mBusyDay[j-1]) {
				int grayLevel=Math.max(0, 180-20*colorCount);
				paint.setColor(Color.rgb(grayLevel,grayLevel,grayLevel));
			}else {
				paint.setColor(Color.rgb(180, 180, 180));
				colorCount=1;
			}
			float posY=(float)3*computeDayLeftPosition(1)/4;
			float circleSize=(float)computeDayLeftPosition(1)/18;
			canvas.drawCircle(computeDayLeftPosition(pos)+posY, posY, circleSize, paint);
		}
	}
	...
	private void drawClick(Canvas canvas) {
		if (mClickedDayIndex != -1) {
			...
			float oneCell=(float)computeDayLeftPosition(1)/3;
			canvas.drawText("+", r.left+2*oneCell, oneCell, p);
	...
	protected void drawWeekNums(Canvas canvas) {
		...
		if (mShowWeekNum) {
			...
			y = mWeekNumAscentHeight
					+ TOP_PADDING_WEEK_NUMBER;

			canvas.drawText(mDayNumbers[0], x, y,
					mWeekNumPaint);
			numCount++;
			i++;
			todayIndex++;
		...
		y = (mHeight - mMonthNumHeight) / 2
				+ mMonthNumAscentHeight;
		...
		for (; i < numCount; i++) {
			...
			x = computeDayLeftPosition(i - offset)
					- getOffsetByText(mDayNumbers[i],
							mMonthNumPaint);

			canvas.drawText(mDayNumbers[i], x, y,
					mMonthNumPaint);
			if (isBold) {
	...
	protected void drawEvents(Canvas canvas) {
		...
		for (ArrayList<Event> eventDay : mEvents) {
			...
			// Determine if everything will fit when
			...
			int remaining = eventDay.size()
					- eventCount;
			if (remaining > 0) {
				drawMoreEvents(canvas,
						remaining,
						xSquare);
			}
		}
	...
	/**
	 * Attempts to draw the given event. Returns the y for the next
	 * event or the original y if the event will not fit. An event is
	 * considered to not fit if the event and its extras won't fit or if
	 * there are more events and the more events line would not fit
	 * after drawing this event.
	 * 
	 * @param canvas
	 ...
	 *                    the event to draw
	 * @param x
	 *                    the top left corner for this event's color
	 *                    chip
	 * @param y
	 *                    the top left corner for this event's color
	 *                    chip
	 * @param rightEdge
	 *                    the rightmost point we're allowed to draw on
	 *                    (exclusive)
	 * @param moreEvents
	 *                    indicates whether additional events will
	 *                    follow this one
	 * @param showTimes
	 *                    if set, a second line with a time range will
	 *                    be displayed for non-all-day events
	 * @param doDraw
	 *                    if set, do the actual drawing; otherwise this
	 *                    just computes the height and returns
	 * @return the y for the next event or the original y if it won't
	 *         fit
	 */
	protected int drawEvent(Canvas canvas, Event event, int x, int y,
			int rightEdge, boolean moreEvents,
			boolean showTimes, boolean doDraw) {
		/*
		 * Vertical layout: (top of box) a.
		 * EVENT_Y_OFFSET_LANDSCAPE or portrait equivalent b.
		 * Event title: mEventHeight for a normal event, +
		 * 2xBORDER_SPACE for all-day event c. [optional] Time
		 * range (mExtrasHeight) d. EVENT_LINE_PADDING
		 * 
		 * Repeat (b,c,d) as needed and space allows. If we have
		 * more events than fit, we need to leave room for
		 * something like "+2" at the bottom:
		 * 
		 ...
		 * (bottom of box)
		 */
		final int BORDER_SPACE = EVENT_SQUARE_BORDER + 1; // want
							// a
							// 1-pixel
							// gap
							// inside
							// border
		final int STROKE_WIDTH_ADJ = EVENT_SQUARE_BORDER / 2; // adjust
							// bounds
							// for
							// stroke
							// width
		...
		if (allDay) {
			// Add a few pixels for the box we draw
			// around all-day events.
			eventRequiredSpace += BORDER_SPACE * 2;
		} else if (showTimes) {
			// Need room for the "1pm - 2pm" line.
			eventRequiredSpace += mExtrasHeight;
		}
		int reservedSpace = EVENT_BOTTOM_PADDING; // leave a bit
						// of
						// room
						// at
						// the
						// bottom
		if (moreEvents) {
			// More events follow. Leave a bit of space
			// between events.
			eventRequiredSpace += EVENT_LINE_PADDING;

			// Make sure we have room for the "+ more"
			// line. (The "+ more" line is expected
			// to be <= the height of an event line, so
			// we won't show "+1" when we could be
			// showing the event.)
		...
		if (allDay) {
			// We shift the render offset "inward",
			// because drawRect with a stroke width
			// greater
			// than 1 draws outside the specified
			// bounds. (We don't adjust the left edge,
			// since
			// we want to match the existing appearance
			// of the "event square".)
			r.left = x;
			r.right = rightEdge - STROKE_WIDTH_ADJ;
			r.top = y + STROKE_WIDTH_ADJ;
			r.bottom = y + mEventHeight + BORDER_SPACE
					* 2
					- STROKE_WIDTH_ADJ;
		...
		CharSequence text = TextUtils.ellipsize(event.title,
				mEventPaint, avail,
		...
		if (showTimes && !allDay) {
			...
			canvas.drawText(text.toString(),
					textX,
					textY,
					isDeclined ? mEventDeclinedExtrasPaint
							: mEventExtrasPaint);
			y += mExtrasHeight;
		}
	...
	protected void drawMoreEvents(Canvas canvas, int remainingEvents,
			int x) {
		int y = mHeight
				- (mExtrasDescent + EVENT_BOTTOM_PADDING);
		String text = getContext()
				.getResources()
				.getQuantityString(R.plurals.month_more_events,
						remainingEvents);
		mEventExtrasPaint.setAntiAlias(true);
		mEventExtrasPaint.setFakeBoldText(true);
		canvas.drawText(String.format(text, remainingEvents),
				x, y, mEventExtrasPaint);
		mEventExtrasPaint.setFakeBoldText(false);
	}

	/**
	 * Draws a line showing busy times in each day of week The method
	 * draws non-conflicting times in the event color and times with
	 * conflicting events in the dna conflict color defined in colors.
	 * 
	 * @param canvas
	 */
	...
	protected void updateSelectionPositions() {
		if (mHasSelectedDay) {
			int selectedPosition = mSelectedDay
					- mWeekStart;
			...
			mSelectedLeft = selectedPosition
					* effectiveWidth
					/ mNumDays
					+ mPadding;
			mSelectedRight = (selectedPosition + 1)
					* effectiveWidth
					/ mNumDays
					+ mPadding;
	...
	public int getDayIndexFromLocation(float x) {
		int dayStart = mShowWeekNum ? SPACING_WEEK_NUMBER
				+ mPadding : mPadding;
		if (x < dayStart || x > mWidth - mPadding) {
			return -1;
		}
		// Selection is (x - start) / (pixels/day) == (x -s) *
		// day / pixels
		return ((int) ((x - dayStart) * mNumDays / (mWidth
				- dayStart - mPadding)));
	}
	...
	public boolean onHoverEvent(MotionEvent event) {
		...
		// only send accessibility events if accessibility and
		// exploration are
		// on.
		...
		if (event.getAction() != MotionEvent.ACTION_HOVER_EXIT) {
			Time hover = getDayFromLocation(event
					.getX());
			if (hover != null
					&& (mLastHoverTime == null || Time
							.compare(hover,
									mLastHoverTime) != 0)) {
				Long millis = hover.toMillis(true);
				String date = Utils.formatDateRange(
						context,
						millis,
						millis,
				...
				if (mShowDetailsInMonth
						&& mEvents != null) {
					int dayStart = SPACING_WEEK_NUMBER
							+ mPadding;
					int dayPosition = (int) ((event
							.getX() - dayStart)
							* mNumDays / (mWidth
							- dayStart - mPadding));
					ArrayList<Event> events = mEvents
							.get(dayPosition);
					List<CharSequence> text = accessEvent
							.getText();
					for (Event e : events) {
						...
						text.add(Utils.formatDateRange(
								context,
								e.startMillis,
								e.endMillis,
								flags)
	...
	public void setBusyDay(ArrayList<Integer> mBusyDayList) {
		mBusyDay = new int[mBusyDayList.size()];
		for (int i = 0; i < mBusyDay.length; i++) {
			mBusyDay[i]=mBusyDayList.get(i);
		}
	}
